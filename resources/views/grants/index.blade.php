            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        {{ date('d M. Y', strtotime($grant->applicationOpen)) }} to {{ date('d M. Y', strtotime($grant->applicationClosed)) }}  <strong class="text-success">(Open)</strong>
                        @elseif ($grant->applicationOpen > date('Y-m-d'))
                        {{ date('d M. Y', strtotime($grant->applicationOpen)) }} to {{ date('d M. Y', strtotime($grant->applicationClosed)) }}  <strong class="text-info">(Upcoming)</strong>
                        @else
                        {{ date('d M. Y', strtotime($grant->applicationOpen)) }} to {{ date('d M. Y', strtotime($grant->applicationClosed)) }} <strong class="text-danger">(Closed)</strong>
                        @endif
                    </td>
                    <td>

                        <a href="{{ route('applications.show',$grant->id) }}" class="btn btn-primary btn-sm mr-1">Applications</a>
